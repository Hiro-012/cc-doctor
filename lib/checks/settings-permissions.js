...
async function isGitTracked(root, relPath) {
  try {
    await execFileAsync('git', ['ls-files', '--error-unmatch', relPath], { cwd: root });
    return true;
  } catch {
    return false; // not tracked, or not a git repo
  }
}

async function checkSettingsFile(root, relPath, report, { warnIfTracked = false } = {}) {
  const fullPath = join(root, relPath);
  if (!existsSync(fullPath)) return;

  if (warnIfTracked && (await isGitTracked(root, relPath))) {
    report.warn(
      'settings-permissions',
      `${relPath} is committed to git. This file is meant for personal, machine-local overrides ` +
        `(by Claude Code convention it belongs in .gitignore) — committing it can leak one contributor's ` +
        `permission grants or local paths to everyone who clones the repo.`,
      relPath
    );
  }
